 ...
 */
    class UserM { 
        ...
        public function account() {
            $id = $_SESSION['user_id'];
            $res = PdoM::Instance() -> Select("SELECT * FROM users WHERE id=" . $id);
        ...
        public function getUserOrders($id) {
            if($id){
                $userOrders = PdoM::Instance() -> Select("SELECT id_order, date_order, status_order FROM orders WHERE id_user=" . $id . " ORDER BY id_order DESC"); //заказы одного пользователя
            } else {
                $userOrders = PdoM::Instance() -> Select("SELECT id_order, date_order, status_order FROM orders ORDER BY id_order DESC"); //заказы всех пользователей
            }
        ...
        public function changeOrderStatus(){
            $id = $_POST['id'];
            $action = $_POST['action'];
            $res = PdoM::Instance() -> Update('orders', ['status_order'=>$action], '`orders`.`id_order` = '.$id);
        }
        ...
        public function changeFeedbackStatus(){
            $id = $_POST['id'];
            $action = $_POST['action'];
            $res = PdoM::Instance() -> Update('feedbacks', ['status'=>$action], '`feedbacks`.`id` = '.$id);
        }
        ...
        public function addToCatalog(){
            ...
            $res = PdoM::Instance() -> Insert('goods', [
            ...
            if($res){                
                $goodId = PdoM::Instance() -> lastIndex(); // Получаем id нового товара
                $idImgName = $goodId.'.'.end($endName); // Делаем имя картинки в карточке товара такое же как и id товара
                $res = PdoM::Instance() -> Update('goods', ['img'=>$idImgName], '`goods`.`id` = '.$goodId);
                if($res){
        ...
        public function editGood(){
            ...
            if (is_uploaded_file($_FILES['img']['tmp_name'])){ // Если загружено новое изображение, получаем имя файла старого изображения и удаляем его
                $getImg = PdoM::Instance() -> Select("SELECT img FROM goods WHERE id='".$_POST['id']."'");
                ...
                $res = PdoM::Instance() -> Update('goods', $vars, '`goods`.`id` = '.$_POST['id']);
            } 
            else {
                $vars = [
                    'category'=>$_POST['category'],
        ...
        public function addFeedback(){
            $res = PdoM::Instance() -> Insert('feedbacks', [
